...
void potentiometer_init(void) {
    adc1_config_width(ADC_WIDTH_BIT_12); //12-bit width for 0-4095 range
    adc1_config_channel_atten(POT1_CHANNEL, ADC_ATTEN_DB_11); //3.3V max input
...
int potentiometer_read(int pot_num) {
    switch (pot_num) {
        case 1: return adc1_get_raw(POT1_CHANNEL); //read raw ADC
